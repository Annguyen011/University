...
class DBManager:
    ...
    def init_db(self):
        ...
        cursor = conn.cursor()
        cursor.execute('''CREATE TABLE IF NOT EXISTS stats (id INTEGER PRIMARY KEY, total_sessions INTEGER, streak INTEGER, last_date TEXT)''')
        cursor.execute('''CREATE TABLE IF NOT EXISTS highscores (game_name TEXT PRIMARY KEY, score INTEGER)''')
        
        # Bảng V3 mới để chứa thêm EXP
    ...
    def update_session(self):
        ...
        total += 1
        conn = sqlite3.connect(self.db_path)
        conn.cursor().execute("UPDATE stats SET total_sessions=?, streak=?, last_date=? WHERE id=1", (total, streak, today.strftime("%Y-%m-%d")))
        conn.commit()
        conn.close()
        return streak, total
    ...
    def increment_score(self, game_name):
        ...
        conn = sqlite3.connect(self.db_path)
        conn.cursor().execute("INSERT OR REPLACE INTO highscores (game_name, score) VALUES (?, ?)", (game_name, new_val))
        conn.commit()
        conn.close()
        return new_val
...
class MiniGame:
    def __init__(self, canvas, width, height, app=None):
        self.canvas, self.width, self.height, self.app = canvas, width, height, app
        self.active, self.after_id = False, None
    def start(self): self.reset(); self.active = True; self.update()
    def stop(self):
        self.active = False
        if self.after_id: self.canvas.after_cancel(self.after_id); self.after_id = None
    def pause(self): self.stop()
    def resume(self):
        if not self.active: self.active = True; self.update()
    def reset(self): pass
    def update(self): pass

# --- Game 1, 2, 3, 4 (Các game chill cơ bản) ---
class DinoRunnerGame(MiniGame):
    def __init__(self, canvas, width, height, app):
        super().__init__(canvas, width, height, app)
        self.COLOR = "#535353"  # Màu chuẩn Chrome
    ...
    def draw_dino(self, x, y):
        c = self.COLOR
        # Thân & Đầu (Tỉ lệ chuẩn pixel)
        self.canvas.create_rectangle(x+22, y, x+44, y+16, fill=c, outline="") # Đầu
        self.canvas.create_rectangle(x+24, y+2, x+28, y+6, fill=self.BG_COLOR, outline="") # Mắt
        self.canvas.create_rectangle(x+2, y+16, x+24, y+32, fill=c, outline="") # Thân
        self.canvas.create_rectangle(x, y+18, x+5, y+28, fill=c, outline="") # Đuôi
        self.canvas.create_rectangle(x+24, y+16, x+30, y+24, fill=c, outline="") # Cổ
        self.canvas.create_rectangle(x+24, y+24, x+35, y+28, fill=c, outline="") # Tay múp
        ...
        if self.dino["jumping"]:
            ...
            self.canvas.create_rectangle(x+10, y+32, x+18, y+36, fill=c, outline="") # Chân co
            self.canvas.create_rectangle(x+20, y+32, x+26, y+44, fill=c, outline="") # Chân trụ
            self.canvas.create_rectangle(x+26, y+40, x+32, y+44, fill=c, outline="") # Bàn chân
        else:
            self.canvas.create_rectangle(x+10, y+32, x+16, y+44, fill=c, outline="") # Chân trụ
            self.canvas.create_rectangle(x+10, y+40, x+16, y+44, fill=c, outline="") # Bàn chân
            self.canvas.create_rectangle(x+18, y+32, x+26, y+36, fill=c, outline="") # Chân co

    def draw_cactus(self, x, y, size_type):
        c = self.COLOR
        # size_type: 0: nhỏ, 1: lớn
        if size_type == 0: # Xương rồng nhỏ
            self.canvas.create_rectangle(x+5, y, x+10, y+30, fill=c, outline="") # Thân
            self.canvas.create_rectangle(x, y+5, x+5, y+15, fill=c, outline="") # Nhánh trái
            self.canvas.create_rectangle(x+10, y+10, x+15, y+20, fill=c, outline="") # Nhánh phải
        else: # Xương rồng lớn
            self.canvas.create_rectangle(x+7, y, x+15, y+45, fill=c, outline="") # Thân
            self.canvas.create_rectangle(x, y+10, x+7, y+25, fill=c, outline="") # Nhánh trái
            self.canvas.create_rectangle(x+15, y+15, x+22, y+30, fill=c, outline="") # Nhánh phải

    def draw_bird(self, x, y):
        c = self.COLOR
        wing_up = (self.frame_count // 10) % 2
        self.canvas.create_rectangle(x+10, y+5, x+30, y+15, fill=c, outline="") # Thân
        self.canvas.create_rectangle(x, y+8, x+10, y+12, fill=c, outline="") # Mỏ
        if wing_up:
            self.canvas.create_polygon(x+15, y+5, x+25, y+5, x+20, y-10, fill=c) # Cánh trên
        else:
            self.canvas.create_polygon(x+15, y+15, x+25, y+15, x+20, y+25, fill=c) # Cánh dưới
    ...
    def update(self):
        ...
        for p in self.ground_pixels:
            p["x"] -= self.speed
            if p["x"] < 0: p["x"] = self.width + 10
            self.canvas.create_rectangle(p["x"], p["y"], p["x"]+2, p["y"]+1, fill=self.COLOR, outline="")

        # Mây
        for cl in self.clouds:
            cl["x"] -= self.speed * 0.1
            if cl["x"] < -60: cl["x"] = self.width + 20; cl["y"] = random.randint(30, 80)
        ...
        if not self.dino["jumping"]:
            for obs in self.obstacles:
                ...
                if 0 < dist < (50 + self.speed * 8):
                    # Chỉ nhảy nếu vật cản ở tầm thấp
                    if obs["type"] != "bird" or obs["y"] > self.ground_y - 50:
                        self.dino["vy"] = -13
                        self.dino["jumping"] = True; break
        ...
        if self.frame_count % random.randint(50, 100) == 0:
            r = random.random()
            if self.score > 500 and r > 0.8: # Chim
            ...
            elif r > 0.4: # Xương rồng nhỏ (cụm 1-3)
        ...
        for obs in self.obstacles[:]:
            ...
            if obs["x"] + obs["w"] < 0:
                ...
                self.score += 1
                self.speed += 0.01 # Tăng tốc cực nhỏ mỗi lần vượt qua

            # Va chạm (Hitbox chuẩn)
            ...
            if (dx + 10 < obs["x"] + obs["w"] and dx + 35 > obs["x"] and 
                ...
                # Game Over Text
                self.canvas.create_text(self.width/2, self.height/2, text="G A M E  O V E R", font=("Courier", 15, "bold"), fill=self.COLOR)
                self.canvas.after(1500, self.start)
                return

        self.draw_dino(self.dino["x"], self.dino["y"])
        
        # Điểm số định dạng 00000
        score_str = f"HI {int(self.high_score):05d}  {int(self.score):05d}"
        self.canvas.create_text(self.width-15, 20, text=score_str, font=("Courier", 10, "bold"), fill=self.COLOR, anchor="ne")
    ...
    def __init__(self, canvas, width, height, app):
        ...
        self.obstacles, self.ground_dots = [], []
        self.speed, self.spawn_timer, self.score, self.frame = 5, 0, 0, 0
        self.high_score = self.app.db.get_highscore("dino_runner") if self.app else 0

    def reset(self):
        self.dino.update({"y": self.ground_y - self.dino["h"], "vy": 0, "jumping": False})
        self.obstacles.clear()
        self.speed, self.spawn_timer, self.score, self.frame = 5, 0, 0, 0
        self.ground_dots = [{"x": random.randint(0, self.width), "y": random.randint(self.ground_y + 2, self.ground_y + 10)} for _ in range(15)]
        if self.app: self.high_score = self.app.db.get_highscore("dino_runner")

    def update(self):
        if not self.active: return
        self.canvas.delete("all")
        self.canvas.create_rectangle(0, 0, self.width, self.height, fill="#F7F7F7", outline="")
        self.canvas.create_line(0, self.ground_y, self.width, self.ground_y, fill="#535353", width=1)
        
        for dot in self.ground_dots:
            dot["x"] -= self.speed
            if dot["x"] < 0: dot["x"] = self.width + random.randint(0, 50)
        ...
        if self.spawn_timer >= 60:
            ...
            w, h = (15, 33) if c == "small1" else (30, 33) if c == "small2" else (23, 46)
            self.obstacles.append({"x": self.width, "y": self.ground_y - h, "w": w, "h": h, "type": c})
            self.spawn_timer, self.speed = random.randint(-20, 10), min(12, self.speed + 0.05)

        for obs in self.obstacles[:]:
            obs["x"] -= self.speed
            if obs["x"] + obs["w"] < 0: self.obstacles.remove(obs); self.score += 10
        ...
        if not self.dino["jumping"]:
            for obs in self.obstacles:
                if obs["x"] + obs["w"] > self.dino["x"] and (obs["x"] - (self.dino["x"] + self.dino["w"])) < 35 + self.speed * 7:
                    self.dino.update({"vy": -14, "jumping": True}); break 

        for obs in self.obstacles:
            if (self.dino["x"]+5 < obs["x"]+obs["w"] and self.dino["x"]+self.dino["w"]-5 > obs["x"] and 
                self.dino["y"]+5 < obs["y"]+obs["h"] and self.dino["y"]+self.dino["h"]-5 > obs["y"]):
                self.active = False
                if self.app and self.score > self.high_score: 
                    self.app.db.increment_score("dino_runner")
                self.canvas.create_text(self.width/2, self.height/2, text="GAME OVER", font=("Courier", 14, "bold"), fill="#535353")
                self.canvas.after(2000, self.start)
                return

        x, y, c = self.dino["x"], self.dino["y"], "#535353"
        ...
        self.canvas.create_rectangle(x, y+18, x+10, y+28, fill=c, outline="")
        ...
        for o in self.obstacles: self.canvas.create_rectangle(o["x"], o["y"], o["x"]+o["w"], o["y"]+o["h"], fill=c, outline="")
        self.canvas.create_text(self.width-80, 20, text=f"HI {self.high_score:05d} | {self.score:05d}", font=("Courier", 10, "bold"), fill="#535353", anchor="e")
...
class AutoPongGame(MiniGame):
    ...
    def update(self):
        ...
        self.ball_x += self.ball_vx; self.ball_y += self.ball_vy
        if self.ball_y - self.ball_r <= 0 or self.ball_y + self.ball_r >= self.height: self.ball_vy = -self.ball_vy
        t1, t2 = (self.height/2 - 20, self.ball_y - 20) if self.ball_vx > 0 else (self.ball_y - 20, self.height/2 - 20)
        self.pad1_y += (t1 - self.pad1_y) * 0.15; self.pad2_y += (t2 - self.pad2_y) * 0.15
        self.pad1_y, self.pad2_y = max(0, min(self.height - 40, self.pad1_y)), max(0, min(self.height - 40, self.pad2_y))
        if self.ball_x - 5 <= 23 and self.pad1_y <= self.ball_y <= self.pad1_y + 40: self.ball_x, self.ball_vx = 28, -self.ball_vx
        if self.ball_x + 5 >= self.width - 23 and self.pad2_y <= self.ball_y <= self.pad2_y + 40: self.ball_x, self.ball_vx = self.width - 28, -self.ball_vx
...
class AquariumGame(MiniGame):
    def __init__(self, canvas, width, height, app):
        ...
        self.reset()
    def reset(self):
        self.fishes = [{"x": random.randint(50, self.width - 50), "y": random.randint(30, self.height - 80), "vx": random.uniform(0.5, 1.4) * random.choice([-1, 1]), "vy": random.uniform(-0.2, 0.2), "color": random.choice(["#FF7F50", "#F4D03F", "#5DADE2", "#F369B4", "#82E0AA", "#E67E22"]), "size": random.randint(18, 30), "wiggle": random.uniform(0, 6)} for _ in range(5)]
        self.bubbles, self.anim_time = [], 0
    def update(self):
        if not self.active: return
        self.canvas.delete("all"); self.anim_time += 0.05
        ...
        for f in self.fishes:
            f["x"] += f["vx"]; f["y"] += f["vy"] + math.sin(self.anim_time * 3 + f["wiggle"]) * 0.15
            if f["x"] > self.width - 30 or f["x"] < 30: f["vx"] = -f["vx"]
            if f["y"] < 20 or f["y"] > self.height - 50: f["vy"] = -f["vy"]
            f["wiggle"] += 0.3
            dir, s, c, tw = 1 if f["vx"] > 0 else -1, f["size"], f["color"], math.sin(f["wiggle"]) * 5.0 
            tx = f["x"] - dir * s * 0.8
            self.canvas.create_polygon(tx, f["y"], tx - dir * s * 0.6, f["y"] - s * 0.45 + tw, tx - dir * s * 0.5, f["y"], tx - dir * s * 0.6, f["y"] + s * 0.45 - tw, fill=c, outline="#1C2833", width=1, smooth=True)
            self.canvas.create_oval(f["x"] - s, f["y"] - s*0.6, f["x"] + s, f["y"] + s*0.6, fill=c, outline="#1C2833", width=1)
            ex = f["x"] + dir * s * 0.6
...
class SnowfallGame(MiniGame):
    def __init__(self, canvas, width, height, app):
        ...
        self.reset()
    def reset(self):
        self.flakes = [{"x": random.randint(0, self.width), "y": random.randint(0, self.height), "vy": random.uniform(1, 3), "r": random.uniform(1.5, 4)} for _ in range(50)]
    def update(self):
        if not self.active: return
        self.canvas.delete("all")
        ...
        for f in self.flakes:
            ...
            if f["y"] > self.height + 10: f["y"], f["x"] = -10, random.randint(0, self.width)
        self.after_id = self.canvas.after(30, self.update)


# --- Game 5: AGE OF WAR THU NHỎ ---
...
TOWER_DATA = {
    ...
    1: {"cost": 100, "dmg": 8,   "cd": 60,  "range": 90,   "color": "#F39C12"},   # Trụ gỗ
    2: {"cost": 300, "dmg": 20,  "cd": 70,  "range": 110,  "color": "#E67E22"},   # Trụ đá
    3: {"cost": 800, "dmg": 50,  "cd": 60,  "range": 130,  "color": "#2ECC71"},   # Trụ thép
    4: {"cost": 99999,"dmg": 120, "cd": 50,  "range": 150,  "color": "#E74C3C"}    # Trụ năng lượng
}
...
class AgeOfWarGame(MiniGame):
    ...
    def reset(self, full_reset=True):
        ...
        if full_reset:
            ...
            if r_hp <= 0: r_hp = AGE_DATA[r_age]["base_hp"]
            if b_hp <= 0: b_hp = AGE_DATA[b_age]["base_hp"]
    ...
    # ------------------ Các phương thức cũ (giữ nguyên logic, chỉ sửa XP) ------------------
    ...
    def add_xp(self, team, amount):
        ...
        if base["xp"] >= req_xp:
            ...
            self.effects.append({"x": base["x"], "y": base["y"]-60, "text": f"LÊN ĐỜI {base['age']}!", "life": 50, "color": "#F1C40F", "size": 16})

    def update(self):
        if not self.active: return
        self.canvas.delete("all")
        
        # Nền Pixel Art (có thể cải tiến thêm)
    ...
    def handle_ai_and_mana(self):
        ...
        for team, base in self.bases.items():
            ...
            if base["summon_cd"] > 0: base["summon_cd"] -= 1
            ...
            if base["summon_cd"] <= 0:
                ...
                if closest_dist > 120 and mana >= tower_cost and tower_lvl < 4:
                    base["tower"] += 1; base["mana"] -= tower_cost
                    self.effects.append({"x": base["x"], "y": base["y"]-60, "text": "+1 TRỤ", "life": 40, "color": "#2ECC71", "size": 14})
                    continue

                if not any(u["type"] == "tank" for u in allies) and mana >= UNIT_BASE["tank"]["cost"]:
                    self.spawn_unit(team, "tank"); continue
                if len(enemies) >= 3 and mana >= UNIT_BASE["mage"]["cost"]:
                    self.spawn_unit(team, "mage"); continue
                if closest_dist < 80 and mana >= UNIT_BASE["sword"]["cost"]:
                    self.spawn_unit(team, "sword"); continue
                if mana >= 60:
                    options = [k for k, v in UNIT_BASE.items() if mana >= v["cost"]]
                    if options: self.spawn_unit(team, random.choice(options))
    ...
    def spawn_unit(self, team, u_type):
        ...
        base["summon_cd"] = 30 
        self.units.append({
            "team": team, "type": u_type, "age": base["age"], "x": base["x"] + base["dir"]*25, "y": self.height - 30,
            "hp": stats["hp"] * mult, "max_hp": stats["hp"] * mult, "dmg": stats["dmg"] * mult, 
            "range": stats["range"], "speed": stats["speed"], "cd": 0, "max_cd": stats["cd"], "action": "walk"
        })
    ...
    def handle_towers(self):
        ...
        for team, base in self.bases.items():
            t_lvl = base["tower"]
            if t_lvl == 0 or base["hp"] <= 0: continue
            if self.tower_cds[team] > 0:
                self.tower_cds[team] -= 1; continue
            ...
            for e in enemies:
                d = abs(base["x"] - e["x"])
                if d <= min_d: min_d, target = d, e
                
            if target:
                self.projectiles.append({
                    "x": base["x"] + base["dir"]*15, "y": base["y"] - 30, 
                    "vx": base["dir"] * 10, "team": team, 
                    "dmg": tower_stats["dmg"] * mult, "aoe": (t_lvl >= 3), "is_tower": True, "age": base["age"]
                })
                self.tower_cds[team] = tower_stats["cd"]

    def handle_logic(self):
        # SỬA: giảm XP từ 5 xuống 4 khi đánh trúng base
        for u in self.units:
            ...
            for e in enemies:
                d = abs(u["x"] - e["x"])
                if d < min_dist: min_dist, closest = d, e
                ...
                if u["cd"] <= 0:
                    ...
                    if u["type"] in ["sword", "tank", "assa"]: 
                        ...
                        if closest == enemy_base:
                            # XP tấn công trụ giảm từ 5 -> 4
                            self.add_xp(u["team"], 4)
                            self.add_xp(enemy_team, 7)
                        self.effects.append({"x": closest["x"], "y": closest["y"]-15, "text": f"-{int(u['dmg'])}", "life": 10, "color": "white", "size": 10})
                    else: 
                        self.projectiles.append({"x": u["x"], "y": u["y"]-10, "vx": (1 if u["team"] == "red" else -1)*8, "team": u["team"], "dmg": u["dmg"], "aoe": UNIT_BASE[u["type"]].get("aoe", False), "age": u["age"]})
                else: u["cd"] -= 1
            else:
                u["action"] = "walk"
    ...
    def update_physics(self):
        # SỬA: XP đạn bắn trúng trụ cũng giảm 5 -> 4
        alive = []
        for u in self.units:
            if u["hp"] > 0: alive.append(u)
            else: self.add_xp("blue" if u["team"] == "red" else "red", 1)
        self.units = alive
        ...
        for proj in self.projectiles[:]:
            ...
            if abs(proj["x"] - enemy_base["x"]) < 25:
                ...
                # XP đạn trúng trụ giảm 5 -> 4
                self.add_xp(proj["team"], 4)
                self.add_xp(enemy_team, 7)
                self.effects.append({"x": proj["x"], "y": enemy_base["y"]-20, "text": f"-{int(proj['dmg'])}", "life": 10, "color": "yellow", "size": 12})
                hit = True
                        ...
                        if proj.get("aoe"):
                            for target in self.units:
                                if target["team"] == enemy_team and abs(proj["x"] - target["x"]) < 40:
                                    target["hp"] -= proj["dmg"]
                                    self.effects.append({"x": target["x"], "y": target["y"]-15, "text": f"-{int(proj['dmg'])}", "life": 10, "color": "#00FFFF", "size": 10})
                        else:
                            e["hp"] -= proj["dmg"]
                            self.effects.append({"x": e["x"], "y": e["y"]-15, "text": f"-{int(proj['dmg'])}", "life": 10, "color": "white", "size": 10})
                        break
    ...
    def draw_world(self):
        ...
        for u in self.units:
            ...
            if age >= 3:
                self.canvas.create_rectangle(x-3, y-22, x+3, y-18, fill="red", outline="black")  # chóp mũ
            ...
            if t == "sword":
                if u["action"] == "attack" and u["cd"] > u["max_cd"] - 5:
                    ...
                    self.canvas.create_oval(x+dir*12, y-8, x+dir*16, y-2, fill="white", outline="")
                else:
                ...
                self.canvas.create_text(x+dir*9, y-5, text="🛡️", font=("Arial", 8))
            elif t == "assa":
                if u["action"] == "attack" and u["cd"] > u["max_cd"] - 5:
        ...
        for name, b in self.bases.items():
            ...
            if b["hp"] > 0:
                # Nhà theo đời với chi tiết đẹp mắt
                if b["age"] == 1:
                    self.canvas.create_polygon(b["x"]-30, b["y"]+30, b["x"], b["y"]-20, b["x"]+30, b["y"]+30, fill=st["b_color"], outline="black", width=2)
                    self.canvas.create_rectangle(b["x"]-10, b["y"]+5, b["x"]+10, b["y"]+25, fill="#5D3A1A", outline="black")
                elif b["age"] == 2:
                    self.canvas.create_rectangle(b["x"]-25, b["y"]-15, b["x"]+25, b["y"]+30, fill=st["b_color"], outline="black", width=2)
                    for i in range(-25, 25, 15):
                        self.canvas.create_rectangle(b["x"]+i, b["y"]-25, b["x"]+i+10, b["y"]-15, fill=st["b_color"], outline="black", width=2)
                    self.canvas.create_rectangle(b["x"]-8, b["y"]-10, b["x"]+8, b["y"]+10, fill="#2C3E50", outline="black")
                    self.canvas.create_line(b["x"], b["y"]-30, b["x"], b["y"]+30, fill="gold", width=2)
                elif b["age"] == 3:
                    self.canvas.create_rectangle(b["x"]-30, b["y"], b["x"]+30, b["y"]+30, fill=st["b_color"], outline="black", width=2)
                    self.canvas.create_rectangle(b["x"]-20, b["y"]-15, b["x"]+20, b["y"], fill=st["b_color"], outline="black", width=2)
                    for i in range(-20, 21, 10):
                        self.canvas.create_rectangle(b["x"]+i, b["y"]-5, b["x"]+i+5, b["y"]+5, fill="#333", outline="black")
                else:
                    self.canvas.create_polygon(b["x"]-20, b["y"]+30, b["x"]-30, b["y"], b["x"]-10, b["y"]-30, b["x"]+10, b["y"]-30, b["x"]+30, b["y"], b["x"]+20, b["y"]+30, fill=st["b_color"], outline="black", width=2)
                    self.canvas.create_oval(b["x"]-8, b["y"]-20, b["x"]+8, b["y"], fill="cyan", outline="white")
                
                # Trụ phòng thủ
                if b["tower"] > 0:
                    t_color = TOWER_DATA[b["tower"]].get("color", "#FFF")
                    self.canvas.create_rectangle(b["x"]-10, b["y"]-45, b["x"]+10, b["y"]-30, fill=t_color, outline="black", width=2)
                    self.canvas.create_rectangle(b["x"]-4, b["y"]-50, b["x"]+4, b["y"]-45, fill="red", outline="black")
                    self.canvas.create_line(b["x"]+b["dir"]*10, b["y"]-37, b["x"]+b["dir"]*22, b["y"]-37, fill="yellow", width=3)
                
                # Thanh máu nhà
                max_hp = AGE_DATA[b["age"]]["base_hp"]
                self.canvas.create_rectangle(b["x"]-25, b["y"]-55, b["x"]+25, b["y"]-50, fill="#641E16", outline="")
                self.canvas.create_rectangle(b["x"]-25, b["y"]-55, b["x"]-25 + max(0, (b["hp"] / max_hp) * 50), b["y"]-50, fill="#2ECC71", outline="")
                # Cờ phe
                self.canvas.create_rectangle(b["x"]-5, b["y"]-60, b["x"]+5, b["y"]-45, fill=b["color"], outline="black")
            else:
                # Nhà đổ
                self.canvas.create_rectangle(b["x"]-25, b["y"]+15, b["x"]+25, b["y"]+30, fill="#7F8C8D", outline="black", width=2)
                self.canvas.create_text(b["x"], b["y"], text="💀", font=("Arial", 20), fill="gray")
        ...
        for proj in self.projectiles:
            ...
            # Vệt sáng
            self.canvas.create_oval(proj["x"]-6, proj["y"]-4, proj["x"]+6, proj["y"]+4, fill="", outline="yellow")
    ...
    def draw_ui(self):
        # (giữ nguyên)
        self.canvas.create_text(self.width/2, 20, text=f"ĐỎ: {self.red_wins}   |   XANH: {self.blue_wins}", fill="#F1C40F", font=("Courier", 14, "bold"))
        r, b = self.bases["red"], self.bases["blue"]
    ...
    def check_win(self):
        ...
        if self.bases["red"]["hp"] <= 0 or self.bases["blue"]["hp"] <= 0:
            ...
            if self.bases["red"]["hp"] <= 0:
                winner = "XANH THẮNG!"
                if self.app: self.blue_wins = self.app.db.increment_score("blue_wins")
            else:
                winner = "ĐỎ THẮNG!"
                if self.app: self.red_wins = self.app.db.increment_score("red_wins")
                
            self.effects.append({"x": self.width/2, "y": self.height/2, "text": winner, "life": 60, "color": "#F1C40F", "size": 24})
            # Reset hoàn toàn sau khi thua (về đời 1)
    ...
    def reset(self):
        ...
        self.bases = {
            "red": {"x": 35, "y": self.height - 40, "dir": 1, "color": "#E74C3C", "hp": r_hp, "mana": r_mana, "age": r_age, "tower": r_tower, "xp": r_xp, "summon_cd": 0},
            "blue": {"x": self.width - 35, "y": self.height - 40, "dir": -1, "color": "#3498DB", "hp": b_hp, "mana": b_mana, "age": b_age, "tower": b_tower, "xp": b_xp, "summon_cd": 0}
        }
    ...
    def save_state_to_db(self):
        if not self.app: return
        r, b = self.bases["red"], self.bases["blue"]
        self.app.db.save_aow_state(r["hp"], r["mana"], r["age"], r["tower"], r["xp"], b["hp"], b["mana"], b["age"], b["tower"], b["xp"])
    ...
    def update(self):
        ...
        # Nền Pixel Art
        self.canvas.create_rectangle(0, 0, self.width, self.height, fill="#87CEEB", outline="") # Bầu trời xanh
        self.canvas.create_rectangle(0, self.height - 30, self.width, self.height, fill="#27AE60", outline="") # Đất
        for i in range(0, self.width, 25): # Cỏ nhấp nhô
            self.canvas.create_rectangle(i, self.height - 32, i+6, self.height - 30, fill="#229954", outline="")
    ...
    def handle_ai_and_mana(self):
        for team, base in self.bases.items():
            ...
            if base["summon_cd"] <= 0:
                ...
                # AI Nâng cấp trụ (Không tốn EXP, chỉ tốn Mana)
                if closest_dist > 120 and mana >= tower_cost and tower_lvl < 4:
                    base["tower"] += 1; base["mana"] -= tower_cost
                    self.effects.append({"x": base["x"], "y": base["y"]-60, "text": "+1 TRỤ", "life": 40, "color": "#2ECC71", "size": 14})
                    continue
        ...
        for u in self.units:
            ...
            if min_dist <= u["range"]:
                ...
                if u["cd"] <= 0:
                    ...
                    if u["type"] in ["sword", "tank", "assa"]: 
                        closest["hp"] -= u["dmg"]
                        if closest == enemy_base: # Đánh trúng nhà
                            self.add_xp(u["team"], 5)
                            self.add_xp(enemy_team, 7)
                        self.effects.append({"x": closest["x"], "y": closest["y"]-15, "text": f"-{int(u['dmg'])}", "life": 10, "color": "white", "size": 10})
                    else: 
                        self.projectiles.append({"x": u["x"], "y": u["y"]-10, "vx": (1 if u["team"] == "red" else -1)*8, "team": u["team"], "dmg": u["dmg"], "aoe": UNIT_BASE[u["type"]].get("aoe", False), "age": u["age"]})
    ...
    def update_physics(self):
        ...
        for u in self.units:
            if u["hp"] > 0: alive.append(u)
            else: self.add_xp("blue" if u["team"] == "red" else "red", 1) # Giết 1 lính được 1 điểm
        self.units = alive
                ...
                for e in self.units:
                    if e["team"] == enemy_team and abs(proj["x"] - e["x"]) < 15:
                        ...
                        if proj.get("aoe"):
                            ...
                            self.effects.append({"x": e["x"], "y": e["y"]-15, "text": f"-{int(proj['dmg'])}", "life": 10, "color": "white", "size": 10})
                        break
            if hit or proj["x"] < 0 or proj["x"] > self.width: self.projectiles.remove(proj)
    ...
    def draw_world(self):
        # MÀU SẮC THEO ĐỜI (AGE)
        def get_age_style(age):
            if age == 1: return {"b_color": "#D35400", "head": "#F5CBA7", "body": "#A04000", "wpn": "#8B4513"} # Lều đá, Da thú
            elif age == 2: return {"b_color": "#95A5A6", "head": "#BDC3C7", "body": "#7F8C8D", "wpn": "#FFFFFF"} # Lâu đài, Giáp sắt
            elif age == 3: return {"b_color": "#1E8449", "head": "#F5CBA7", "body": "#117A65", "wpn": "#17202A"} # Boong ke, Lính rằn ri
            else: return {"b_color": "#8E44AD", "head": "#9B59B6", "body": "#2C3E50", "wpn": "#00FFFF"} # Tháp tương lai, Mech suit
        ...
        for u in self.units:
            ...
            # Đầu và Thân
            self.canvas.create_rectangle(x-4, y-15, x+4, y-7, fill=st["head"], outline="black") 
            self.canvas.create_rectangle(x-5, y-7, x+5, y+2, fill=st["body"], outline="black") 
            self.canvas.create_rectangle(x-4, y+2, x-2, y+8, fill="#5D6D7E", outline="black")
            self.canvas.create_rectangle(x+2, y+2, x+4, y+8, fill="#5D6D7E", outline="black")
            
            # Vũ khí theo class
            if t == "sword":
                if u["action"] == "attack" and u["cd"] > u["max_cd"] - 5:
                    self.canvas.create_rectangle(x, y-5, x+dir*15, y-3, fill=st["wpn"], outline="black") 
                else: 
                    self.canvas.create_rectangle(x+dir*4, y-10, x+dir*6, y-2, fill=st["wpn"], outline="black")
            elif t == "bow":
                self.canvas.create_rectangle(x+dir*4, y-12, x+dir*6, y-2, fill=st["wpn"], outline="black")
            elif t == "tank":
                self.canvas.create_rectangle(x+dir*5, y-10, x+dir*10, y+4, fill=st["head"], outline="black") # Khiên bự
            elif t == "assa":
                if u["action"] == "attack" and u["cd"] > u["max_cd"] - 5:
                    self.canvas.create_rectangle(x, y-2, x+dir*12, y, fill="red", outline="black") 
            elif t == "mage":
                self.canvas.create_rectangle(x-6, y-20, x+6, y-14, fill=st["body"], outline="black") # Nón
                self.canvas.create_rectangle(x+dir*8-3, y-12-3, x+dir*8+3, y-12+3, fill=st["wpn"], outline="black") # Quả cầu phép
            
            # Máu lính
            self.canvas.create_rectangle(x-8, y-25, x+8, y-23, fill="#C0392B", outline="")
            self.canvas.create_rectangle(x-8, y-25, x-8 + max(0,(u["hp"]/u["max_hp"])*16), y-23, fill="#2ECC71", outline="")
        ...
        for name, b in self.bases.items():
            ...
            if b["hp"] > 0:
                if b["age"] == 1:
                    ...
                    # Lâu đài răng cưa
                    self.canvas.create_rectangle(b["x"]-25, b["y"]-15, b["x"]+25, b["y"]+30, fill=st["b_color"], outline="black", width=2)
                    for i in range(-25, 25, 15): self.canvas.create_rectangle(b["x"]+i, b["y"]-25, b["x"]+i+10, b["y"]-15, fill=st["b_color"], outline="black", width=2)
                elif b["age"] == 3:
                    # Boong-ke mái vòm (pixel vòm)
                    self.canvas.create_rectangle(b["x"]-30, b["y"], b["x"]+30, b["y"]+30, fill=st["b_color"], outline="black", width=2)
                    self.canvas.create_rectangle(b["x"]-20, b["y"]-15, b["x"]+20, b["y"], fill=st["b_color"], outline="black", width=2)
                else:
                    # Tháp tương lai hình thoi
                    self.canvas.create_polygon(b["x"]-20, b["y"]+30, b["x"]-30, b["y"], b["x"]-10, b["y"]-30, b["x"]+10, b["y"]-30, b["x"]+30, b["y"], b["x"]+20, b["y"]+30, fill=st["b_color"], outline="black", width=2)
                
                # Trụ phòng thủ
                if b["tower"] > 0:
                    t_c = TOWER_DATA[b["tower"]].get("color", "#FFF")
                    self.canvas.create_rectangle(b["x"]-8, b["y"]-45, b["x"]+8, b["y"]-30, fill=t_c, outline="black", width=2)
                    self.canvas.create_rectangle(b["x"]+(b["dir"]*8), b["y"]-41, b["x"]+(b["dir"]*20), b["y"]-35, fill="#FDFEFE", outline="black")
                
                # Thanh máu & Cờ
                self.canvas.create_rectangle(b["x"]-5, b["y"]-5, b["x"]+5, b["y"]+15, fill=b["color"], outline="black") # Cờ phe
                max_hp = AGE_DATA[b["age"]]["base_hp"]
                self.canvas.create_rectangle(b["x"]-25, b["y"]-55, b["x"]+25, b["y"]-50, fill="#641E16", outline="")
                self.canvas.create_rectangle(b["x"]-25, b["y"]-55, b["x"]-25 + max(0, (b["hp"] / max_hp) * 50), b["y"]-50, fill="#2ECC71", outline="")
            else:
        ...
        for proj in self.projectiles:
            c = "#00FFFF" if proj.get("aoe") else get_age_style(proj.get("age", 1))["wpn"]
            if proj.get("is_tower"): c = TOWER_DATA.get(self.bases[proj["team"]]["tower"], {}).get("color", "#FFF")
            self.canvas.create_rectangle(proj["x"]-3, proj["y"]-2, proj["x"]+3, proj["y"]+2, fill=c, outline="black")
    ...
    def check_win(self):
        ...
        if self.bases["red"]["hp"] <= 0 or self.bases["blue"]["hp"] <= 0:
            ...
            self.effects.append({"x": self.width/2, "y": self.height/2, "text": winner, "life": 60, "color": "#F1C40F", "size": 24})
            self.save_state_to_db() 
            self.canvas.after(3000, self.reset)

# =========================== ỨNG DỤNG ĐỒNG HỒ TỔNG ===========================
class StudyTimer:
    ...
    def build_ui(self):
        ...
        self.stats_label = tk.Label(left_frame, text=f"🔥 Chuỗi: {streak} ngày | 📚 Tổng: {total} lần", font=("Segoe UI", 10, "bold"), fg="#F39C12", bg=self.bg_color)
    ...
    def init_game(self, game_key):
        if self.current_game:
            if isinstance(self.current_game, AgeOfWarGame): self.current_game.save_state_to_db()
            self.current_game.stop()
            
        w, h = 300, 200
        if game_key == "dino_runner": game = DinoRunnerGame(self.game_canvas, w, h, self)
        elif game_key == "age_of_war": game = AgeOfWarGame(self.game_canvas, w, h, self)
        elif game_key == "auto_pong": game = AutoPongGame(self.game_canvas, w, h, self)
        elif game_key == "aquarium": game = AquariumGame(self.game_canvas, w, h, self)
        elif game_key == "snowfall": game = SnowfallGame(self.game_canvas, w, h, self)
        else: return
